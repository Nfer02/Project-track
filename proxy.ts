...
const PUBLIC_PATHS = ["/login", "/register", "/forgot-password", "/reset-password", "/faq", "/terms", "/privacy"]
const PUBLIC_EXACT = ["/"]
...
export const config = {
  matcher: [
    "/((?!api|_next/static|_next/image|favicon.ico|sitemap.xml|robots.txt|google.*\\.html|.*\\.mp4$|.*\\.png$|.*\\.jpg$|.*\\.svg$|mockups).*)",
  ],
}
