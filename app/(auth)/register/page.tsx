...
const features = [
  ...
  "Carga de facturas con lectura automatica (IA)",
...
export default async function RegisterPage({ searchParams }: Props) {
  ...
  // If no email param, redirect to waitlist
  if (!email) {
    redirect("/#waitlist")
  }

  // Check if email is invited
  const invited = await prisma.waitlist.findFirst({
    where: { email, invited: true },
  })
  ...
  return (
    <div className="grid min-h-svh lg:grid-cols-[520px_1fr]">
      ...
      <div className="flex flex-col items-center justify-center px-6 py-12 bg-background">
        ...
        <RegisterForm invitedEmail={email} />
      </div>
    </div>
  )
}
